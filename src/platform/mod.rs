...
impl SystemResources {
    ...
    /// Formula: j = 1 + c/m where m = memory pressure (min 0.01), c = cpu scaling factor
    /// Higher memory pressure reduces jobs (larger m = smaller c/m = fewer jobs)
    pub fn calculate_optimal_jobs(&self, max_jobs: Option<u32>) -> u32 {
        ...
        let cpu_scaling_factor = if self.memory_pressure > 0.7 {
            ...
            if let Some(logger) = crate::logger::Logger::global() {
                logger.trace(&format!("High memory pressure (>0.7): CPU scaling factor = {} * 0.5 = {}", self.cpu_cores, factor), None);
            }
            factor
            ...
            if let Some(logger) = crate::logger::Logger::global() {
                logger.trace(&format!("Moderate memory pressure (>0.4): CPU scaling factor = {} * 0.75 = {}", self.cpu_cores, factor), None);
            }
            factor
        } else {
            ...
            if let Some(logger) = crate::logger::Logger::global() {
                logger.trace(&format!("Low memory pressure (≤0.4): CPU scaling factor = {} * 1.0 = {}", self.cpu_cores, factor), None);
            }
            factor
        };

        // Calculate optimal jobs: j = 1 + c/m
        ...
        let final_jobs = if let Some(max) = max_jobs {
            ...
            if let Some(logger) = crate::logger::Logger::global() {
                logger.trace(&format!("User max limit: {} -> min({}) = {}", optimal_jobs, max, limited), None);
            }
            limited
        } else {
            if let Some(logger) = crate::logger::Logger::global() {
                logger.trace(&format!("No user limit, final jobs: {}", optimal_jobs), None);
            }
            optimal_jobs
        };
...
pub fn system_resources() -> Result<&'static SystemResources, PlatformError> {
    SYSTEM_RESOURCES.get_or_init(|| SystemResources::detect().unwrap_or_else(|_| {
    ...
    // Return a reference to the cached resources
...
pub fn refresh_system_resources() -> Result<&'static SystemResources, PlatformError> {
    // Note: OnceLock doesn't support refresh, so we'd need a different approach
    // For now, detection happens once per program run which is usually sufficient
    system_resources()
}
...
fn detect_cpu_cores() -> Result<u32, PlatformError> {
    ...
    // Try sysconf(_SC_NPROCESSORS_ONLN) first
...
// Runtime dispatch for Unix family with platform-specific optimizations
...
fn detect_memory_info_macos() -> Result<(u64, u64), PlatformError> {
    ...
    if let Ok(output) = Command::new("vm_stat").output() {
        if output.status.success() {
            ...
            if pages_active > 0 || pages_free > 0 {
                ...
                let page_size = if std::env::consts::ARCH == "aarch64" { 16384 } else { 4096 };
    ...
    let mut count = std::mem::size_of::<libc::vm_statistics64>() as libc::mach_msg_type_number_t / std::mem::size_of::<libc::integer_t>() as libc::mach_msg_type_number_t;
...
fn detect_cpu_cores() -> Result<u32, PlatformError> {
    ...
    if logical_processors > 0 {
        ...
        std::thread::available_parallelism()
            .map(|n| n.get() as u32)
            .map_err(|e| PlatformError::CpuDetectionFailed {
                reason: format!("GetSystemInfo returned 0 and available_parallelism failed: {}", e),
            })
    }
}
...
mod tests {
    ...
    fn test_system_resources_detection() {
        ...
        #[cfg(any(target_os = "linux", target_os = "macos", target_os = "windows"))]
        {
            ...
            assert!(resources.memory_pressure >= 0.0 && resources.memory_pressure <= 1.0, 
    ...
    fn test_memory_pressure_edge_cases() {
        ...
        assert!(jobs >= 1, "Should still allow at least 1 job under high pressure");
