...
#[command(name = "empack")]
#[command(about = "Minecraft modpack management")]
#[command(version)]
...
mod tests {
    ...
    fn test_cli_parsing_basic_commands() {
        ...
        let cli = Cli::try_parse_from(&["empack", "requirements"]).unwrap();
        assert!(matches!(cli.command, Some(Commands::Requirements)));

        let cli = Cli::try_parse_from(&["empack", "init"]).unwrap();
        assert!(matches!(cli.command, Some(Commands::Init { .. })));
    }

    #[test]
    fn test_cli_parsing_with_args() {
        let cli = Cli::try_parse_from(&["empack", "init", "my-pack", "--force"]).unwrap();
        if let Some(Commands::Init { name, force }) = cli.command {
            assert_eq!(name, Some("my-pack".to_string()));
            assert!(force);
        } else {
            panic!("Expected Init command");
        }

        let cli = Cli::try_parse_from(&["empack", "build", "mrpack", "client", "--clean"]).unwrap();
        if let Some(Commands::Build { targets, clean, .. }) = cli.command {
            assert_eq!(targets, vec!["mrpack", "client"]);
            assert!(clean);
        } else {
    ...
    #[test]
    fn test_cli_global_config_integration() {
        ...
        assert_eq!(cli.config.color, TerminalCapsDetectIntent::Always);
    }

    #[test]
    fn test_command_requirements_check() {
        ...
        assert!(Commands::Sync { dry_run: false }.requires_modpack());
        assert!(
            Commands::Build {
                targets: vec![],
                clean: false,
