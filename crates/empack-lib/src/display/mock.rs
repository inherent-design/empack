...
impl DisplayProvider for MockDisplayProvider {
    ...
    fn prompt(&self) -> Box<dyn PromptProvider> {
        Box::new(MockPromptProvider::new(self.calls.clone(), self.responses.clone()))
    }
...
pub enum DisplayCall {
    // Status calls
    StatusChecking { task: String },
    StatusSuccess { item: String, details: String },
    StatusError { item: String, details: String },
    StatusWarning { message: String },
    StatusInfo { message: String },
    StatusMessage { text: String },
    StatusEmphasis { text: String },
    StatusSubtle { text: String },
    StatusList { items: Vec<String> },
    StatusComplete { task: String },
    StatusToolCheck { tool: String, available: bool, version: String },
    StatusSection { title: String },
    StatusStep { current: usize, total: usize, description: String },
    
    // Progress calls
    ProgressBar { total: u64 },
    ProgressSpinner { message: String },
    ProgressSetPosition { tracker_id: usize, pos: u64 },
    ProgressInc { tracker_id: usize },
    ProgressIncBy { tracker_id: usize, n: u64 },
    ProgressSetMessage { tracker_id: usize, message: String },
    ProgressTick { tracker_id: usize, item: String },
    ProgressFinish { tracker_id: usize, message: String },
    ProgressAbandon { tracker_id: usize, message: String },
    
    // Prompt calls
    PromptConfirm { message: String, response: bool },
    PromptInput { message: String, response: Option<String> },
    PromptSelect { message: String, options: Vec<String>, response: Option<usize> },
    PromptMultiSelect { message: String, options: Vec<String>, response: Vec<usize> },
    
    // Structured calls
    StructuredTable { headers: Vec<String>, rows: Vec<Vec<String>> },
    StructuredList { items: Vec<String> },
    StructuredProperties { pairs: Vec<(String, String)> },
}
...
impl StatusProvider for MockStatusProvider {
    fn checking(&self, task: &str) {
        self.record_call(DisplayCall::StatusChecking { task: task.to_string() });
    }
    ...
    fn warning(&self, message: &str) {
        self.record_call(DisplayCall::StatusWarning { message: message.to_string() });
    }
    
    fn info(&self, message: &str) {
        self.record_call(DisplayCall::StatusInfo { message: message.to_string() });
    }
    
    fn message(&self, text: &str) {
        self.record_call(DisplayCall::StatusMessage { text: text.to_string() });
    }
    
    fn emphasis(&self, text: &str) {
        self.record_call(DisplayCall::StatusEmphasis { text: text.to_string() });
    }
    
    fn subtle(&self, text: &str) {
        self.record_call(DisplayCall::StatusSubtle { text: text.to_string() });
    }
    ...
    fn complete(&self, task: &str) {
        self.record_call(DisplayCall::StatusComplete { task: task.to_string() });
    }
    ...
    fn section(&self, title: &str) {
        self.record_call(DisplayCall::StatusSection { title: title.to_string() });
    }
...
impl ProgressProvider for MockProgressProvider {
    fn bar(&self, total: u64) -> Box<dyn ProgressTracker> {
        ...
        self.calls.lock().unwrap().push(DisplayCall::ProgressBar { total });
        Box::new(MockProgressTracker::new(tracker_id, self.calls.clone()))
    }
    
    fn spinner(&self, message: &str) -> Box<dyn ProgressTracker> {
        let tracker_id = self.get_next_tracker_id();
        self.calls.lock().unwrap().push(DisplayCall::ProgressSpinner { 
...
impl ProgressTracker for MockProgressTracker {
    fn set_position(&self, pos: u64) {
        self.calls.lock().unwrap().push(DisplayCall::ProgressSetPosition { 
            tracker_id: self.id, 
            pos 
        });
    }
    ...
    fn set_message(&self, message: &str) {
        self.calls.lock().unwrap().push(DisplayCall::ProgressSetMessage { 
            tracker_id: self.id, 
            message: message.to_string() 
        });
    }
    ...
    fn finish(&self, message: &str) {
        self.calls.lock().unwrap().push(DisplayCall::ProgressFinish { 
            tracker_id: self.id, 
            message: message.to_string() 
        });
    }
    ...
    fn abandon(&self, message: &str) {
        self.calls.lock().unwrap().push(DisplayCall::ProgressAbandon { 
            tracker_id: self.id, 
            message: message.to_string() 
        });
    }
...
impl MockPromptProvider {
    fn new(calls: Arc<Mutex<Vec<DisplayCall>>>, responses: Arc<Mutex<HashMap<String, ResponseValue>>>) -> Self {
...
impl PromptProvider for MockPromptProvider {
    ...
    fn multi_select(&self, message: &str, options: &[&str]) -> Vec<usize> {
        ...
        self.calls.lock().unwrap().push(DisplayCall::PromptMultiSelect { 
            message: message.to_string(),
            options: options.iter().map(|s| s.to_string()).collect(),
            response: response.clone(),
        });
...
impl StructuredProvider for MockStructuredProvider {
    fn table(&self, headers: &[&str], rows: &[Vec<&str>]) {
        self.calls.lock().unwrap().push(DisplayCall::StructuredTable { 
            headers: headers.iter().map(|s| s.to_string()).collect(),
            rows: rows.iter().map(|row| row.iter().map(|s| s.to_string()).collect()).collect(),
        });
    }
    
    fn list(&self, items: &[&str]) {
        self.calls.lock().unwrap().push(DisplayCall::StructuredList { 
            items: items.iter().map(|s| s.to_string()).collect(),
        });
    }
    
    fn properties(&self, pairs: &[(&str, &str)]) {
        self.calls.lock().unwrap().push(DisplayCall::StructuredProperties { 
